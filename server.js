...
const issuesdb = JSON.parse(fs.readFileSync('./api/db.json', 'UTF-8'))
...
server.get('/api/users', (req, res) => {
  const users = userdb.users.map(({password, ...result}) => result)
  return res.status(200).json({users})
})

server.get('/api/issues', (req, res) => {
  return res.status(200).json({issues: issuesdb.issues})
})
...
server.post('/auth/register', (req, res) => {
  ...
  fs.readFile("./api/users.json", (err, data) => {
    ...
    var writeData = fs.writeFile("./api/users.json", JSON.stringify(data), (err, result) => {  // WRITE
      if (err) {
        const status = 401
        const message = err
        res.status(status).json({status, message})
